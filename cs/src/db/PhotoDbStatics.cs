...
public class PhotoDbStatics
{
  ...
  public static void CreatePhotoDb(string path)
  {
    using (var connection = CreateConnection(path))
    {
      ...
      DbMigration.Migrate(userVersion, connection);
  ...
  public static void CreateTable(SqliteConnection connection, string commandText)
  {
    var command = connection.CreateCommand();
  ...
  public static bool AddColumn(SqliteConnection connection, string table, string column, string type)
  {
    try
    {
      var command = connection.CreateCommand();
  ...
  public static void CreateIndex(SqliteConnection connection, string commandText)
  {
    var command = connection.CreateCommand();
    command.CommandText = commandText;
    using (var reader = command.ExecuteReader())
